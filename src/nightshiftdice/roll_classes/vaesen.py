...
class Vaesen(RollClass):
    ...
    def write_inits_to_file(self) -> None:
        with open('vaesen_inits.json', 'w') as i:
            json.dump(self.inits, i)

    def read_inits_from_file(self) -> bool:
        if not os.path.exists('vaesen_inits.json'):
            return False
        with open('vaesen_inits.json', 'r') as i:
...
/v 66                   Roll a d66 by Vaesen's logic```""")
        ...
        if re.match(r'^start( \d+)?$', self.dice_str):
            ...
            shuffle(numbers)
            self.inits = dict(sorted({name: numbers[i] for i, name in enumerate(names)}.items(), key=lambda x: x[1]))
        ...
        if self.dice_str == 'end':
            if not os.path.exists('vaesen_inits.json'):
                ...
                return
            os.remove('vaesen_inits.json')
            await self._say('Combat **ended**!')
            return
